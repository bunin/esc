...
func Run(conf *Config) error {
	...
	w := os.Stdout
	wData := w
	pkgHeaderText, err := render(conf.Package, !(conf.Private), pkgHeader)
	if nil != err {
	...
	headerText, err := render(conf.Package, !(conf.Private), headerTemplate)
	if nil != err {
		return fmt.Errorf("failed to expand autogenerated code: %s", err)
	}
...
func render(packageName string, enableExports bool, tpl string) ([]byte, error) {
	if tpl == "" {
		tpl = headerTemplate
	}
	functionPrefix := ""
...
const (
	pkgHeader = `// Code generated by "esc {{.Invocation}}"; DO NOT EDIT.
