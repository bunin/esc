...
type Config struct {
	...
	NoCompression bool
	// BuildTags, build tags to add to generated files.
	BuildTags string
...
func Run(conf *Config) error {
	...
	w := os.Stdout
	wData := w
	pkgHeaderText, err := render(conf.Package, !(conf.Private), conf.BuildTags, pkgHeader)
	if nil != err {
		return fmt.Errorf("failed to expand autogenerated code: %s", err)
	}
	if conf.OutputFile != "" {
		w, err = os.Create(conf.OutputFile)
		if err != nil {
			return fmt.Errorf("failed to create output file: %s", err)
		}
		ext := filepath.Ext(conf.OutputFile)
		wData, err = os.Create(conf.OutputFile[:len(conf.OutputFile)-len(ext)] + "-data" + ext)
		if err != nil {
			return fmt.Errorf("failed to create output file: %s", err)
		}
		if _, err := wData.Write(pkgHeaderText); err != nil {
			return fmt.Errorf("failed to write output: %s", err)
		}
	}
	headerText, err := render(conf.Package, !(conf.Private), conf.BuildTags, headerTemplate)
	if nil != err {
		return fmt.Errorf("failed to expand autogenerated code: %s", err)
	}
	if _, err := w.Write(pkgHeaderText); err != nil {
		return fmt.Errorf("failed to write output: %s", err)
	}
	if _, err := w.Write(headerText); err != nil {
		return fmt.Errorf("failed to write output: %s", err)
	}
	if _, err := wData.WriteString(dataHeader); err != nil {
		return fmt.Errorf("failed to write output: %s", err)
	}
	...
	if _, err = wData.WriteString(footer); err != nil {
...
func render(packageName string, enableExports bool, buildTags string, tpl string) ([]byte, error) {
	if tpl == "" {
		tpl = headerTemplate
	}
	functionPrefix := ""
...
const (
	pkgHeader = `{{ if .BuildTags }}// +build {{.BuildTags}}
{{end}}// Code generated by "esc {{.Invocation}}"; DO NOT EDIT.
